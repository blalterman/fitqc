...
print(f"{'=' * 100}")
print("COMPREHENSIVE FILTERING AUDIT - ALL PPA12 DATASETS")
print(f"{'=' * 100}\n")
...
for dataset_name in datasets:
    ...
    if not datasets_identical:
        print("  ✗ Filtered datasets are not identical")
...
for result in summary_results:
    ...
    print(
        f"{result['dataset']:<12} {result['n_total']:>10,} {result['n_below_L']:>10,} {result['n_above_U']:>10,} "
...
if oob_datasets:
    ...
    for r in oob_datasets:
        print(
            f"  - {r['dataset']}: {r['n_out_of_bounds']:,} samples ({r['pct_out_of_bounds']:.2f}%)"
        )
