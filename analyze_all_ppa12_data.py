...
def analyze_dataset(name: str):
    ...
    for tol in tolerances:
        ...
        if count_lower > 0:
            print(
                f"    tol={tol:.2e}: {count_lower:>6,} samples ({frac_lower:>7.4f}), excess={excess_lower:>6.1f}x"
            )

    print("  Upper boundary (u > 1-tol):")
    for tol in tolerances:
        ...
        if count_upper > 0:
            print(
                f"    tol={tol:.2e}: {count_upper:>6,} samples ({frac_upper:>7.4f}), excess={excess_upper:>6.1f}x"
            )
    ...
    if expected.get("expected_lower_stickiness") is not None:
        print(
            f"  Lower: {lower_status} (expected={expected['expected_lower_stickiness']}, got={result.lower_pileup_detected})"
        )
    if expected.get("expected_upper_stickiness") is not None:
        print(
            f"  Upper: {upper_status} (expected={expected['expected_upper_stickiness']}, got={result.upper_pileup_detected})"
        )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print(
        f"{'Dataset':<15} {'Lower Exp':<12} {'Lower Det':<12} {'Upper Exp':<12} {'Upper Det':<12} {'Status'}"
    )
    print("-" * 80)
    ...
    for name in datasets:
        if name in results:
            ...
            print(
                f"{name:<15} {lower_exp:<12} {lower_det:<12} {upper_exp:<12} {upper_det:<12} {status}"
            )
