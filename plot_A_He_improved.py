...
print(f"Generating improved ultra-high-resolution plots for A_He...")
print(f"Using z-order to place reference lines BELOW histogram data")
...
ax1.axvline(L, color='darkred', linestyle='--', linewidth=2.5,
            label=f'L={L}', zorder=1, alpha=0.8)
ax1.axvline(U, color='darkred', linestyle='--', linewidth=2.5,
            label=f'U={U}', zorder=1, alpha=0.8)
if x0 is not None and x0 != L:  # Only draw if x0 doesn't coincide with L
    ax1.axvline(x0, color='darkorange', linestyle=':', linewidth=2.5,
                label=f'x0={x0}', zorder=1, alpha=0.8)

# Draw histogram ABOVE reference lines
counts_full, edges_full, _ = ax1.hist(x, bins=bins_raw_full,
...
# Histogram ABOVE
ax2.hist(x, bins=bins_raw_full, color='steelblue', edgecolor='none',
         alpha=0.85, zorder=2)
...
# Reference lines BELOW
ax4.axvline(0, color='darkred', linestyle='--', linewidth=2.5,
            label='u=0 (L)', zorder=1, alpha=0.8)
ax4.axvline(1, color='darkred', linestyle='--', linewidth=2.5,
            label='u=1 (U)', zorder=1, alpha=0.8)

# Histogram ABOVE with high contrast color
ax4.hist(u, bins=bins_u_full, color='rebeccapurple', edgecolor='none',
         alpha=0.85, zorder=2)
...
# Histogram ABOVE
ax5.hist(u, bins=bins_u_full, color='rebeccapurple', edgecolor='none',
         alpha=0.85, zorder=2)
...
if len(u_zoom_extreme_lower) > 0:
    ...
    ax6.axvline(0.001, color='darkorange', linestyle=':', linewidth=2,
                label='u=0.001', zorder=1, alpha=0.7)
    ax6.axvline(0.01, color='gold', linestyle=':', linewidth=2,
                label='u=0.01', zorder=1, alpha=0.7)

    # Histogram ABOVE
    ax6.hist(u_zoom_extreme_lower, bins=bins_u_extreme_lower,
...
if len(u_zoom_upper) > 0:
    ...
    ax7.axvline(0.999, color='darkorange', linestyle=':', linewidth=2,
                label='u=0.999', zorder=1, alpha=0.7)
    ax7.axvline(0.99, color='gold', linestyle=':', linewidth=2,
                label='u=0.99', zorder=1, alpha=0.7)

    # Histogram ABOVE
    ax7.hist(u_zoom_upper, bins=bins_u_zoom_upper, color='darkslateblue',
             edgecolor='none', alpha=0.85, zorder=2)
...
# Reference lines BELOW
ax8.axvline(0.001, color='darkorange', linestyle=':', linewidth=2,
            label='u=0.001', zorder=1, alpha=0.7)
ax8.axvline(0.01, color='gold', linestyle=':', linewidth=2,
            label='u=0.01', zorder=1, alpha=0.7)
ax8.axhline(near_L_001/len(u), color='darkorange', linestyle=':',
            linewidth=1.5, zorder=1, alpha=0.5)
ax8.axhline(near_L_01/len(u), color='gold', linestyle=':',
            linewidth=1.5, zorder=1, alpha=0.5)
...
# Reference lines BELOW
ax9.axvline(0.999, color='darkorange', linestyle=':', linewidth=2,
            label='u=0.999', zorder=1, alpha=0.7)
ax9.axvline(0.99, color='gold', linestyle=':', linewidth=2,
            label='u=0.99', zorder=1, alpha=0.7)
ax9.axhline(near_U_999/len(u), color='darkorange', linestyle=':',
            linewidth=1.5, zorder=1, alpha=0.5)
ax9.axhline(near_U_99/len(u), color='gold', linestyle=':',
            linewidth=1.5, zorder=1, alpha=0.5)
...
# Reference lines BELOW
ax10.axvline(0.001, color='darkorange', linestyle=':', linewidth=1.5,
             label='u=0.001', zorder=1, alpha=0.5)
ax10.axvline(0.01, color='gold', linestyle=':', linewidth=1.5,
             label='u=0.01', zorder=1, alpha=0.5)
ax10.axvline(0.99, color='gold', linestyle=':', linewidth=1.5,
             label='u=0.99', zorder=1, alpha=0.5)
ax10.axvline(0.999, color='darkorange', linestyle=':', linewidth=1.5,
             label='u=0.999', zorder=1, alpha=0.5)

# Horizontal lines for concentrations
ax10.axhline(near_L_01/len(u), color='red', linestyle='--',
             linewidth=1, zorder=1, alpha=0.3, label=f'Lower: {100*near_L_01/len(u):.2f}%')
ax10.axhline(1 - near_U_99/len(u), color='blue', linestyle='--',
             linewidth=1, zorder=1, alpha=0.3, label=f'Upper: {100*near_U_99/len(u):.2f}%')
...
ax11.text(0.05, 0.5, summary_text_left, fontsize=8.5, family='monospace',
...
ax12.text(0.05, 0.5, summary_text_right, fontsize=8.5, family='monospace',
          verticalalignment='center',
          bbox=dict(boxstyle='round', facecolor='lightcyan', alpha=0.8, edgecolor='gray'))

plt.suptitle('A_He Ultra-High-Resolution Analysis (12 panels: histograms + CDFs)',
