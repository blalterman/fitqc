...
print(f"\n{'=' * 80}")
print("Testing Different Elbow Detection Strategies")
print(f"{'=' * 80}")
...
for curve, direction, log_x in strategies:
    elbow = select_elbow(
        quantile_grid, tol_at_quantile, curve=curve, direction=direction, log_x=log_x
    )
...
for i, (q, t) in enumerate(zip(quantile_grid, tol_at_quantile, strict=False)):
    if t > 1e-6:
        print(f"First non-zero tolerance at index {i}:")
        print(f"  quantile={q:.6f}, tolerance={t:.6f}")
        if i > 0:
            print(
                f"  Previous: quantile={quantile_grid[i - 1]:.6f}, tolerance={tol_at_quantile[i - 1]:.6f}"
            )
        break
...
for i in range(len(quantile_grid)):
    if i > 0:
        ...
        if i < 15:  # First 15 points
            print(
                f"  i={i}: q={quantile_grid[i]:.6f}, ratio={ratios[i]:.2f}, change={ratio_change:.3f}"
            )
