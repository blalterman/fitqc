...
if n_filtered_method1 == n_filtered_method2:
    print("  ✓ Methods produce SAME number of filtered samples")
else:
    print("  ✗ Methods produce DIFFERENT numbers!")
...
if np.array_equal(np.sort(x_filtered_method1), np.sort(x_filtered_method2)):
    print("  ✓ Filtered datasets are IDENTICAL")
else:
    print("  ✗ Filtered datasets are DIFFERENT!")
...
ax.hist(
    u_zoom_lower, bins=bins_u_zoom, alpha=0.7, color="darkgreen", edgecolor="black", linewidth=0.5
)
ax.axvline(0, color="red", linestyle="--", linewidth=2, label="u=0 (L)")
if result.t_lo_star is not None:
    ax.axvline(
