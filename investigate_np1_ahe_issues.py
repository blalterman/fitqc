...
def investigate_np1():
    ...
    print(
        f"  Between L and 10*L: {np.sum(above_L_near):,} ({100 * np.sum(above_L_near) / len(x):.3f}%)"
    )
    ...
    print("When sorted, these become u_sorted[0:n] where u < 0.")
    print("The algorithm detects this as a 'pileup' at the lower boundary.")
    print("\nBut these samples are OUTSIDE the parameter bounds!")
    print("This is either:")
    print("  (a) A data quality issue (samples should be clipped to L)")
    print("  (b) An algorithm issue (should filter u < 0 as invalid)")
    print("  (c) Expected behavior (L is a soft bound, not hard constraint)")
...
def investigate_a_he_upper():
    ...
    print("=" * 80)
    print(
        f"There are {exactly_at_U:,} samples ({100 * exactly_at_U / len(x):.3f}%) exactly at U={U}."
    )
    ...
    print("  - 0.45% pileup is detectable (similar to e_dv_ap's 0.62% at lower)")
    print("  - Algorithm is working correctly")
    print("\nPossible explanations:")
    print("  (a) Metadata is incorrect - there IS upper boundary stickiness")
    print("  (b) 0.45% is below the 'significance threshold' for A_He")
    print("  (c) This is a known false positive case")
