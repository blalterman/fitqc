...
for eps in eps_values:
    ...
    print(f"{eps:<12.2e} {near_L:>6} ({100*near_L/len(x):>5.2f}%)  "
...
for i in range(min(20, len(hist))):
    cumulative += hist[i]
    print(f"{i:<5} [{bin_edges[i]:>8.5f}, {bin_edges[i+1]:>8.5f})  "
...
print(f"{'u threshold':<15} {'P(u < tol)':<15} {'Samples':<12} {'Expected (uniform)':<20} {'Excess'}")
print("-"*90)
...
print(f"  Samples in first bin [0, {bin_width_1000:.4f}): {np.sum(x < bin_width_1000):,} ({100*np.sum(x < bin_width_1000)/len(x):.3f}%)")
