...
if result_tight['single_curve_elbow'] is not None:
    ax.axhline(result_tight['single_curve_elbow'], color='red', ls='--',
               linewidth=2.5, label=f"Single elbow: {result_tight['single_curve_elbow']:.4f}")
...
if result_tight['multi_curve_elbow'] is not None:
    ...
    ax.axhline(multi_thresh, color='blue', ls='--',
               linewidth=2.5, label=f"Multi elbow: {multi_thresh:.4f}")

# Mark ground truth
ax.axhline(0.003, color='green', ls='-.', linewidth=2.5,
           label='True boundary: 0.003', alpha=0.8)

ax.set_xlabel('Quantile', fontsize=12, fontweight='bold')
ax.set_ylabel('Tolerance', fontsize=12, fontweight='bold')
ax.set_title('Boundary: Tight Pileup (3% in 0.3%)',
...
if result_uniform['single_curve_elbow'] is not None:
    ax.axhline(result_uniform['single_curve_elbow'], color='red', ls='--',
               linewidth=2.5, label=f"Single elbow: {result_uniform['single_curve_elbow']:.4f}")
...
ax.set_xlabel('Quantile', fontsize=12, fontweight='bold')
ax.set_ylabel('Tolerance', fontsize=12, fontweight='bold')
ax.set_title('Boundary: Uniform (Should be Linear)',
             fontsize=14, fontweight='bold')
ax.legend(fontsize=10, loc='best')
ax.grid(True, alpha=0.3)
...
if result_spike['single_curve_elbow'] is not None:
    ax.axhline(result_spike['single_curve_elbow'], color='red', ls='--',
               linewidth=2.5, label=f"Single elbow: {result_spike['single_curve_elbow']:.4e}")
...
if result_lognorm['single_curve_elbow'] is not None:
    ax.axhline(result_lognorm['single_curve_elbow'], color='red', ls='--',
               linewidth=2.5, label=f"Single elbow: {result_lognorm['single_curve_elbow']:.4e}")
...
ax.set_xlabel('Quantile (log scale)', fontsize=12, fontweight='bold')
ax.set_ylabel('Epsilon (log scale)', fontsize=12, fontweight='bold')
ax.set_title('Interior: Signed Log-Normal (Close to y=x, No Spike)',
             fontsize=14, fontweight='bold')
ax.legend(fontsize=10, loc='best')
ax.grid(True, alpha=0.3, which='both')
...
print("="*120)
print(f"{'Test Case':<30} | {'Single Elbow':<15} | {'Multi Elbow':<15} | {'True':<20} | {'Single Error':<15} | {'Multi Error':<15}")
print("-"*120)

for row in table_data:
    print(f"{row['Test Case']:<30} | {row['Single Elbow']:<15} | {row['Multi Elbow']:<15} | {row['True']:<20} | {row['Single Error']:<15} | {row['Multi Error']:<15}")
...
ax = axes_detail[0, 0]
ax.plot(quantile_grid, result_tight['diagnostics']['threshold_at_quantile'],
        'o-', linewidth=2, markersize=6, label='Threshold', color='navy', alpha=0.7)
ax.axhline(0.003, color='green', ls='-.', linewidth=2.5, label='True: 0.003', alpha=0.8)
if result_tight['single_curve_elbow'] is not None:
    ax.axhline(result_tight['single_curve_elbow'], color='red', ls='--',
               linewidth=2, label=f"Single: {result_tight['single_curve_elbow']:.4f}")
if result_tight['multi_curve_elbow'] is not None:
    multi_thresh = np.interp(
        result_tight['multi_curve_elbow'],
        quantile_grid,
    ...
    ax.axhline(multi_thresh, color='blue', ls='--',
               linewidth=2, label=f"Multi: {multi_thresh:.4f}")
...
if 'curvature' in result_tight['diagnostics']:
    ax.plot(quantile_grid, result_tight['diagnostics']['curvature'],
            'o-', linewidth=2, markersize=4, label='Curvature', color='darkred')
    ax.set_xlabel('Quantile', fontsize=11)
...
ax = axes_detail[1, 0]
ax.plot(quantile_grid, result_uniform['diagnostics']['threshold_at_quantile'],
        'o-', linewidth=2, markersize=6, label='Threshold', color='purple', alpha=0.7)
...
ax.plot(quantile_grid, residuals + quantile_grid, 's-', linewidth=1, markersize=4,
        label=f'Residual (max={np.max(np.abs(residuals)):.4f})', color='orange', alpha=0.6)
ax.set_xlabel('Quantile', fontsize=11)
ax.set_ylabel('Tolerance', fontsize=11)
...
ax = axes_detail[1, 1]
ax.plot(quantile_grid, result_broad['diagnostics']['threshold_at_quantile'],
        'o-', linewidth=2, markersize=6, label='Threshold', color='darkgreen', alpha=0.7)
ax.axhline(0.05, color='green', ls='-.', linewidth=2.5, label='True: 0.05', alpha=0.8)
if result_broad['single_curve_elbow'] is not None:
    ax.axhline(result_broad['single_curve_elbow'], color='red', ls='--',
               linewidth=2, label=f"Single: {result_broad['single_curve_elbow']:.4f}")
if result_broad['multi_curve_elbow'] is not None:
    multi_thresh = np.interp(
        result_broad['multi_curve_elbow'],
        quantile_grid,
...
threshold_log = result_spike['diagnostics']['threshold_at_quantile'][:len(quantile_log)]
ax.loglog(quantile_log, threshold_log, 'o-', linewidth=2, markersize=6,
          label='Epsilon', color='darkorange', alpha=0.7)
...
ax2 = ax.twinx()
ax2.plot(quantile_log, deviation, 's-', linewidth=1, markersize=4,
         label='Log deviation', color='red', alpha=0.5)
...
threshold_log = result_lognorm['diagnostics']['threshold_at_quantile'][:len(quantile_log)]
ax.loglog(quantile_log, threshold_log, 'o-', linewidth=2, markersize=6,
          label='Epsilon', color='teal', alpha=0.7)
ax.loglog(quantile_log, quantile_log, 'k--', linewidth=2, label='y=x', alpha=0.5)
deviation = np.log10(threshold_log) - np.log10(quantile_log)
ax2 = ax.twinx()
ax2.plot(quantile_log, deviation, 's-', linewidth=1, markersize=4,
         label=f'Log deviation (std={np.std(deviation):.3f})', color='purple', alpha=0.5)
...
if single_error_tight is not None and multi_error_tight is not None:
    if multi_error_tight < single_error_tight:
        ...
        print(f"  Tight (0.3%): Multi-curve is BETTER by {improvement:.1f}% (errors: {single_error_tight:.2%} vs {multi_error_tight:.2%})")
    elif single_error_tight < multi_error_tight:
        degradation = (multi_error_tight - single_error_tight) / multi_error_tight * 100
        print(f"  Tight (0.3%): Single-curve is BETTER by {degradation:.1f}% (errors: {single_error_tight:.2%} vs {multi_error_tight:.2%})")
    else:
        print(f"  Tight (0.3%): Methods are EQUIVALENT (both {single_error_tight:.2%})")
else:
    print(f"  Tight (0.3%): Cannot compare (single: {single_error_tight}, multi: {multi_error_tight})")
...
if single_error_broad is not None and multi_error_broad is not None:
    if multi_error_broad < single_error_broad:
        ...
        print(f"  Broad (5.0%): Multi-curve is BETTER by {improvement:.1f}% (errors: {single_error_broad:.2%} vs {multi_error_broad:.2%})")
    elif single_error_broad < multi_error_broad:
        degradation = (multi_error_broad - single_error_broad) / multi_error_broad * 100
        print(f"  Broad (5.0%): Single-curve is BETTER by {degradation:.1f}% (errors: {single_error_broad:.2%} vs {multi_error_broad:.2%})")
    else:
        print(f"  Broad (5.0%): Methods are EQUIVALENT (both {single_error_broad:.2%})")
else:
    print(f"  Broad (5.0%): Cannot compare (single: {single_error_broad}, multi: {multi_error_broad})")
...
if not uniform_has_elbow:
    ...
    print(f"  Uniform: WARNING - Elbows detected when none expected (single: {single_elbow_uniform}, multi: {multi_elbow_uniform})")
...
if not lognorm_has_elbow:
    ...
    print(f"  Log-Normal: INFO - Elbows detected (single: {single_elbow_lognorm}, multi: {multi_elbow_lognorm})")
...
if spike_detected:
    print(f"  Spike: PASS - Elbow detected (single: {single_elbow_spike}, multi: {multi_elbow_spike})")
else:
