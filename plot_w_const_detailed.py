...
print(f"Generating detailed plots for w_const...")
...
ax1.set_title(f'w_const Full Distribution (including out-of-bounds)\n{len(x):,} total samples', fontsize=13, fontweight='bold')
...
if n_below > 0:
    ...
    ax2.text(0.05, 0.95, f'Samples at exactly 0.0: {n_at_zero}\n({n_at_zero/n_below*100:.1f}% of out-of-bounds)',
             transform=ax2.transAxes, verticalalignment='top',
             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8), fontsize=10)
ax2.set_xlabel('Raw value (x)', fontsize=12)
ax2.set_ylabel('Count', fontsize=12)
ax2.set_title(f'Out-of-Bounds Samples (x < L={L})\n{n_below:,} samples - LIKELY FAILED FITS', fontsize=13, fontweight='bold')
...
ax4.hist(x_valid[mask_upper_zoom], bins=bins_upper_zoom, color='darkorange', edgecolor='none', alpha=0.7)
