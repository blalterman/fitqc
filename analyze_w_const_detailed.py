...
if n_below > 0:
    ...
    if np.sum(x_below == 0.0) > n_below * 0.9:
        print(f"  Most samples at exactly 0.0 → Likely FAILED FITS (similar to np1)")
    else:
        print(f"  Samples distributed in range [{x_below.min():.2f}, {x_below.max():.2f}]")
        print(f"  This may indicate a different kind of data quality issue")
...
if near_U_1e2 / len(u) > 0.005:
    ...
    print(f"  This confirms expected_upper_stickiness=true in metadata")
else:
